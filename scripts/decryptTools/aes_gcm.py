...
class AesGcmDecryptInterceptor:
    ...
    def decrypt_value(self, encrypted_text: str, url: str, field: str, full_json: Dict[str, Any] = None, form_data: str = "") -> str:
        ...
        try:
            ...
            # 记录日志
            if full_json:
                # 如果是JSON数据，创建一个只包含当前字段的对象
                original_value = {field: encrypted_text}
                try:
                    # 尝试解析解密后的值为JSON
                    decrypted_json = json.loads(decrypted_text)
                    processed_value = {field: decrypted_json}
                except json.JSONDecodeError:
                    processed_value = {field: decrypted_text}
            else:
                # 如果是表单数据，直接使用原始值和解密值
                original_value = {field: encrypted_text}
                processed_value = {field: decrypted_text}
            ...
            self.logger.log(None, self.logger._format_log_message(
                url,
                field,
                json.dumps({field: encrypted_text}, ensure_ascii=False),
                json.dumps({field: error_msg}, ensure_ascii=False)
            ))
            raise
    ...
    def process_json_data(self, data: Dict[str, Any], url: str) -> Tuple[Dict[str, Any], bool]:
        ...
        for field in self.decryption_fields:
            if field in data and isinstance(data[field], str):
                try:
                    decrypted_value = self.decrypt_value(data[field], url, field, full_json=data)
                    try:
                        # 尝试将解密后的值解析为JSON
...
# 注册插件
addons = [AesGcmDecryptInterceptor(fields, key, iv)]