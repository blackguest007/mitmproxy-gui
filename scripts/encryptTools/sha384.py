...
使用方法:
    mitmdump -p 8888 -s sha384.py --ssl-insecure field=password
    mitmdump -p 8888 -s sha384.py --ssl-insecure field=password,username
...
注意事项:
    ...
    2. 支持单个或多个字段加密
    3. SHA384加密结果为96位小写字符串
    4. 日志文件保存在 logs 目录下，格式为: encrypt_sha384_时间戳.log
...
class Sha384EncryptInterceptor:
    """SHA384 加密拦截器"""
    ...
    def encrypt_value(self, plain_text: str, url: str, field: str, full_json: Dict[str, Any] = None, form_data: str = "") -> str:
        """加密单个值"""
        try:
            hash_obj = hashlib.sha384(plain_text.encode('utf-8'))
...
# 注册插件
addons = [Sha384EncryptInterceptor(encryption_fields)] 