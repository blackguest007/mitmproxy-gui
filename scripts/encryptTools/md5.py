...
使用方法:
    mitmdump -p 8888 -s md5.py --ssl-insecure field=password
    mitmdump -p 8888 -s md5.py --ssl-insecure field=password,username
...
注意事项:
    ...
    2. 支持单个或多个字段加密
    3. 加密结果为32位小写字符串
    4. 日志文件保存在 src/logs 目录下，格式为: encrypt_md5_时间戳.log
"""
...
class Md5EncryptInterceptor(BaseInterceptor):
    """MD5 加密拦截器"""
    def __init__(self):
    ...
    def encrypt_value(self, value: str, url: str, field: str, full_json: Dict[str, Any] = None, form_data: str = "") -> str:
        ...
        try:
            ...
            self.logger.log(None, f"MD5加密失败: {str(e)}")
            return value

# 创建拦截器实例
interceptor = Md5EncryptInterceptor()
