...
def get_des3_config() -> Tuple[str, bytes]:
    """获取DES3配置"""
    key = None
    iv = None
    for arg in sys.argv:
    ...
    # 验证密钥长度
    if len(key_bytes) != 24:
        raise ValueError(f"无效的DES3密钥长度: {len(key_bytes)}字节，应为24字节")
...
def get_log_filename(fields: List[str]) -> str:
    """生成日志文件名"""
    return f"des3_cbc_{'_'.join(fields)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class Des3CbcEncryptInterceptor:
    """DES3-CBC 加密拦截器"""
    
    def __init__(self, encryption_fields: List[str], key: str, iv: bytes):
...
try:
    fields = get_fields()
    key, iv = get_des3_config()
except Exception as e:
    print(f"配置错误: {str(e)}", file=sys.stderr)
    sys.exit(1)

# 注册插件
addons = [Des3CbcEncryptInterceptor(fields, key, iv)]
