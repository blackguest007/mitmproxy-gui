...
注意事项:
    ...
    3. AES密钥必须是16字节长度
    4. 加密结果使用Base64编码
    5. 日志文件保存在 logs 目录下，格式为: encrypt_aes_ecb_时间戳.log
...
class AesEcbEncryptInterceptor:
    ...
    def __init__(self, encryption_fields: List[str], key: str):
        ...
        # 使用 UTF-8 编码处理密钥，与 CryptoJS.enc.Utf8.parse 保持一致
    ...
    def encrypt_value(self, plain_text: str, url: str, field: str) -> str:
        ...
        try:
            # 使用 ECB 模式和 PKCS7 填充，与 CryptoJS 保持一致
            cipher = AES.new(self.key, AES.MODE_ECB)
            # 使用 PKCS7 填充，与 CryptoJS.pad.Pkcs7 保持一致
            padded_data = pad(plain_text.encode('utf-8'), AES.block_size)
            encrypted_data = cipher.encrypt(padded_data)
            # 使用 Base64 编码，与 CryptoJS 的 toString() 保持一致
                ...
                if current_path in self.encryption_fields:
                    try:
                        ...
                        if isinstance(value, dict):
                            ...
                            modified = True
                            # 记录加密日志，显示完整的JSON数据，使用 separators 参数去除多余的空格
                            self.logger.log(None, self.logger._format_log_message(url, current_path, original_data, json.dumps(data, separators=(',', ':'), ensure_ascii=False)))
                        # 如果值是字符串，直接加密
...
# 获取脚本配置
encryption_fields, key = get_fields(), get_aes_config()
