...
使用方法:
    mitmdump -p 8888 -s aes_ecb.py --ssl-insecure field=password key=your_key
    mitmdump -p 8888 -s aes_ecb.py --ssl-insecure field=password,username key=your_key

参数说明:
    field: 需要处理的字段，多个字段用逗号分隔
    key: AES 密钥（16/24/32字节）

注意事项:
    1. 支持 application/json 和 application/x-www-form-urlencoded 格式
    2. 支持单个或多个字段加密
    3. 加密结果会进行 Base64 编码
    4. 日志文件保存在 src/logs 目录下，格式为: encrypt_aes_ecb_时间戳.log
"""
...
class AesEcbEncryptInterceptor(BaseInterceptor):
    """AES-ECB 加密拦截器"""
    def __init__(self):
    ...
    def encrypt_value(self, value: str, url: str, field: str, full_json: Dict[str, Any] = None, form_data: str = "") -> str:
        ...
        try:
            ...
            self.logger.log(None, f"AES-ECB加密失败: {str(e)}")
            return value

# 创建拦截器实例
interceptor = AesEcbEncryptInterceptor()
