...
使用方法:
    mitmdump -p 8888 -s sha512.py --ssl-insecure field=password
    mitmdump -p 8888 -s sha512.py --ssl-insecure field=password,username
...
注意事项:
    ...
    2. 支持单个或多个字段加密
    3. SHA512加密结果为128位小写字符串
    4. 日志文件保存在 logs 目录下，格式为: encrypt_sha512_时间戳.log
...
class Sha512EncryptInterceptor:
    """SHA512 加密拦截器"""
    ...
    def encrypt_value(self, plain_text: str, url: str, field: str, full_json: Dict[str, Any] = None, form_data: str = "") -> str:
        """加密单个值"""
        try:
            hash_obj = hashlib.sha512(plain_text.encode('utf-8'))
...
# 注册插件
addons = [Sha512EncryptInterceptor(encryption_fields)] 