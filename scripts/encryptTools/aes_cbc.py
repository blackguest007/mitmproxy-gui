...
注意事项:
    ...
    4. 初始化向量必须是16字节长度
    5. 加密结果使用Base64编码
    6. 日志文件保存在 logs 目录下，格式为: encrypt_aes_cbc_时间戳.log
...
LOG_CONFIG = {
    ...
    'sleep_interval': 0.1  # 休眠间隔（秒）
}

def get_script_config() -> Tuple[List[str], Optional[str], Optional[str]]:
    """从命令行参数获取脚本配置"""
    fields = []
    key = None
    iv = None
...
def get_log_filename(fields: List[str]) -> str:
    """生成日志文件名"""
    return f"encrypt_aes_cbc_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class AsyncLogger:
    ...
    def _safe_put_log(self, log_message: str) -> None:
        """安全地添加日志到队列"""
        if not log_message:
            return
            
        try:
            self.log_queue.put_nowait((None, log_message))
        except queue.Full:
            try:
...
class AesCbcEncryptInterceptor:
    ...
    def __init__(self, encryption_fields: List[str], key: str, iv: str):
    ...
    def encrypt_value(self, plain_text: str, url: str, field: str) -> str:
        ...
        try:
            ...
            # 记录加密日志
            self.logger.log(None, self.logger._format_log_message(url, field, plain_text, encrypted_text))
            return encrypted_text
        except Exception as e:
            error_msg = f"加密失败: {str(e)}"
            # 记录错误日志
            self.logger.log(None, self.logger._format_log_message(url, field, plain_text, error_msg))
            raise
            ...
            for key, value in d.items():
                ...
                if current_path in self.encryption_fields:
                    try:
                        ...
                        self.logger.log(None, self.logger._format_log_message(url, current_path, str(value), f"加密失败: {str(e)}"))
...
# 获取脚本配置
encryption_fields, key, iv = get_script_config()
