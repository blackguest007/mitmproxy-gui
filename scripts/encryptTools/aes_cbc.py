...
使用方法:
    ...
    mitmdump -p 8888 -s aes_cbc.py --ssl-insecure field=password,username key=your_key iv=your_iv
...
注意事项:
    ...
    2. 支持单个或多个字段加密
    3. 加密结果会进行 Base64 编码
    4. 日志文件保存在 src/logs 目录下，格式为: encrypt_aes_cbc_时间戳.log
"""
...
class AesCbcEncryptInterceptor(BaseInterceptor):
    """AES-CBC 加密拦截器"""
    def __init__(self):
        script_name = os.path.splitext(os.path.basename(__file__))[0]
        self.key, self.iv = get_aes_config()
        super().__init__(
    ...
    def encrypt_value(self, value: str, url: str, field: str, full_json: Dict[str, Any] = None, form_data: str = "") -> str:
        ...
        try:
            ...
            self.logger.log(None, f"AES-CBC加密失败: {str(e)}")
            return value

# 创建拦截器实例
interceptor = AesCbcEncryptInterceptor()
