...
使用方法:
    解密模式: mitmdump -p 8888 -s des_ecb.py --mode upstream:http://127.0.0.1:8080 --ssl-insecure field=password key=your_key
...
class DesEcbBothInterceptor(BothInterceptor):
    """DES-ECB 双向加解密拦截器"""
...
# 创建拦截器实例
interceptor = DesEcbBothInterceptor()
