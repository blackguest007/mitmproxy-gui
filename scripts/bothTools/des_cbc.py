...
使用方法:
    解密模式: mitmdump -p 8888 -s des_cbc.py --mode upstream:http://127.0.0.1:8080 --ssl-insecure field=password key=your_key iv=your_iv
...
class DesCbcBothInterceptor(BothInterceptor):
    """DES-CBC 双向加解密拦截器"""
        ...
        try:
            ...
            if current_mode == 'decrypt':
                ...
                try:
                    ...
                    if len(encrypted_data) % DES.block_size != 0:
                        return value
...
# 创建拦截器实例
interceptor = DesCbcBothInterceptor()
