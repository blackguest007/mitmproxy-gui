...
【使用方法】
双向模式: mitmdump -p 8888 -s aes_cbc.py --mode upstream:http://127.0.0.1:8080 --ssl-insecure field=password key=your_key iv=your_iv
...
class AesCbcBothInterceptor(BothInterceptor):
    """AES-CBC 双向加解密拦截器"""
...
# 创建拦截器实例
interceptor = AesCbcBothInterceptor()
