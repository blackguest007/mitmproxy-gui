...
from mode.mode import update_mode
from thread.mitmproxy_thread import MitmProxyThread
from validator.KeyValidator import validate_key_length
from validator.PortValidator import PortValidator
from otherTools.rsa_handler import handle_rsa_keys  # 导入新的处理函数
from constants.error_messages import ErrorMessages
from constants.mode_constants import ModeConstants
from constants.encryption_constants import EncryptionConstants
from constants.ui_constants import UIConstants
from constants.message_constants import MessageConstants
    ...
    try:
        ...
        if mode == ModeConstants.DEFAULT:
            ...
            if mode == ModeConstants.ENCRYPT:
                ...
                # 检查密钥长度之前，确保用户选择了脚本
                if not key and encrypt_script not in EncryptionConstants.NO_KEY_ALGORITHMS:
                    window.packet_detail.append(ErrorMessages.KEY_REQUIRED)  # 提示需要密钥
                    return

                # 生成命令
                commands = window.script_loader.generate_command(mode, key=enc_key, iv=iv)

            elif mode == ModeConstants.DECRYPT:

                # 先验证密钥
                if not validate_key_length(window, decrypt_script, key):
                    return

                # 然后检查是否需要处理 RSA 密钥
                if 'rsa.py' in decrypt_script:
                    pem_path, error = handle_rsa_keys(window, mode, key)
                    if error:
                        window.packet_detail.append(ErrorMessages.INVALID_MODE)
                        return
                    key = pem_path  # 使用处理后的密钥路径
                    window.packet_detail.append(MessageConstants.DECRYPT_KEY_SAVED.format(key))
                ...
                # 如果是 RSA 模式，处理密钥
                if 'rsa.py' in encrypt_script or 'rsa.py' in decrypt_script:
                    # 处理加密密钥
                    if 'rsa.py' in encrypt_script:
                        pem_path, error = handle_rsa_keys(window, "Encrypt", key['encrypt'])
                        if error:
                            window.packet_detail.append(ErrorMessages.INVALID_MODE)
                            return
                    ...
                    # 处理解密密钥
                    if 'rsa.py' in decrypt_script:
                        pem_path, error = handle_rsa_keys(window, "Decrypt", key['decrypt'])
                        if error:
                            window.packet_detail.append(ErrorMessages.INVALID_MODE)
                            return
