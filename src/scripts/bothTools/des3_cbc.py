...
class DES3CBCProcessor:
    def __init__(self, fields):
        self.fields = fields
        self.is_encrypt = is_encrypt_mode()
        logging.info(f"初始化 3DES CBC {'加密' if self.is_encrypt else '解密'}处理器")
    ...
    def process_value(self, value: str) -> str:
        ...
        if self.is_encrypt:
            return base64.b64encode(cipher.encrypt(pad(value.encode('utf-8'), DES3.block_size))).decode('utf-8')
        else:
            return unpad(cipher.decrypt(base64.b64decode(value)), DES3.block_size).decode('utf-8')
...
# 获取配置并注册插件
fields = get_fields()
addons = [DES3CBCProcessor(fields)] 